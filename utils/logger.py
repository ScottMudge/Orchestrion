...
def info(message: str, suppress_header: bool = False):
    global queue
    global logger
    global logger_set
    ...
    if not suppress_header:
        header = colored("[{}]".format("General"), name_color) + colored(" INFO", info_level_color) + ": {}"

    if not logger_set:
        entry = LogEntry(header + message, MESSAGE_INFO)
        queue.append(entry)
        return
    logger.logger.info(header + message)


def warn(message: str, suppress_header: bool = False):
    global queue
    global logger
    global logger_set
    ...
    header = ""
    if not suppress_header:
        header = colored("[{}]".format("General"), name_color) + colored(" WARN", warn_level_color) + ": {}"
...
def error(message: str, suppress_header: bool = False):
    global queue
    global logger
    global logger_set
    ...
    header = ""
    if not suppress_header:
        header = colored("[{}]".format("General"), name_color) + colored(" ERR", err_level_color) + ": {}"
...
def debug(message: str, suppress_header: bool = False):
    global queue
    global logger
    global logger_set
    ...
    header = ""
    if not suppress_header:
        header = colored("[{}]".format("General"), name_color) + colored(" DBG", debug_level_color) + ": {}"
...
def _check_queue():
    ...
    for log_entry in queue:
        if log_entry.msg_type == MESSAGE_DEBUG:
            debug(log_entry.message, True)
        elif log_entry.msg_type == MESSAGE_ERROR:
            info(log_entry.message, True)
        elif log_entry.msg_type == MESSAGE_WARN:
            warn(log_entry.message, True)
